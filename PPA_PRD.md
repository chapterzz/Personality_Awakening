...
#### 用户角色与身份（学生 / 管理员）
- **统一用户表**：学生、管理员均为 **`User`** 的一条记录（同一套昵称 + 密码注册体系），通过 **`User.role`** 区分职能（枚举：`STUDENT` | `ADMIN`）。**游客**无 `User` 行，仅有 `TemporarySession`。
- **管理员 (Admin)**：`role = ADMIN`；用于 CMS、运维类能力（与开发任务书 T4.5 等对齐）。**MVP 赋权方式**：默认自助注册为 `STUDENT`；`ADMIN` 由 **Seed、管理员后台提升或运营预置账号** 完成（不在此文档展开运营流程细节）。
...
### 1.3 全局洞察看板 (Global Insights Dashboard)
- **公开页面**: 无需登录即可访问，展示所有注册用户测评结果的汇总统计。
- **可视化组件**:
  - **16 型人格人数分布**: 柱状图展示各 MBTI 类型的人数，按人数降序排列。
  - **类型×精灵热力图**: 矩阵热力图，横轴 16 种 MBTI 类型，纵轴精灵角色（曦光领航精灵/月影探索精灵），颜色深浅表示人数。
  - **趣味数据卡片**: 最常见类型、最稀有类型、最受欢迎精灵、维度平衡度（E/I、S/N、T/F、J/P 各侧占比）。
- **个人对比**（仅登录用户可见）:
  - 展示用户自己的人格类型。
  - 在图表中高亮自己的类型。
  - 文案：”你是 XX 类型，全球 Y% 的人也是这个类型”。
- **数据更新**: 用户访问页面时加载最新数据，无需实时推送。
...
### 2.1 实体说明
- **User**: `id (UUID)`, `nickname`, `password_hash`, **`role` (枚举: `STUDENT` \| `ADMIN`)**, `created_at`, `is_deleted`
  - **`role` 默认值**：新用户自助注册为 `STUDENT`；`ADMIN` 由 Seed、后台赋权或运营配置。
...
### 2.3 权限与鉴权（摘要）
- **JWT**（或会话）载荷至少包含 `user_id` 与 **`role`**；后端 Guard 按接口要求校验。
- **Admin API**（CMS）：须 **`role = ADMIN`**（与 T4.5 一致）。
- **全局洞察看板**：`GET /dashboard/stats` 为公开端点，无需认证；`GET /dashboard/my-comparison` 需 JWT 认证。
...
- **动效**: 
  ...
  - 精灵：Idle 状态下的呼吸灯效果。
- **响应式**:
  - Mobile: 单列布局，底部导航栏 (测试/中心/看板/图书馆)。
...
- [ ] 全局洞察看板无需登录即可访问；16 型人格分布、精灵热力图、趣味数据卡片数据准确；登录用户可看到个人类型与全局对比。
